    ...
    try:
        if "file" not in request.files:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                reconstructor.load(temp_file.name)
...
def get_step(step_index: int):
    ...
    try:
        ...
        # Debug logging
        logger.info(
            f"Step {step_index}: {len(viz_data.nodes)} nodes, {len(viz_data.edges)} edges"
        )
        if viz_data.nodes:
            logger.info(
                f"First node: {viz_data.nodes[0].id} - {viz_data.nodes[0].content_preview}"
            )
...
def get_legend():
    ...
    try:
        ...
        return jsonify({"success": True, "legend": legend_data})
    except Exception as e:
