...
class ConversationBoundary(BaseModel):
    ...
    start_timestamp: datetime = Field(description="Start time of the conversation")
    end_timestamp: datetime = Field(description="End time of the conversation")
    summary: str = Field(description="Compressed summary of the entire conversation")
    embedding_vector: Optional[List[float]] = Field(
        default=None, description="Embedding of the summary for similarity search"
    )
    topic_tags: List[str] = Field(
        default_factory=list, description="Topic tags extracted from the conversation"
    )
...
class SemanticCluster(BaseModel):
    ...
    id: str
    cluster_topic: str = Field(description="Main topic or theme of this cluster")
    summary: str = Field(description="Compressed summary of the cluster content")
    embedding_vector: Optional[List[float]] = Field(
        default=None, description="Embedding of the summary for similarity search"
    )
    ...
    conversation_ids: List[str] = Field(
        default_factory=list, description="Tier 3 conversation IDs in this cluster"
    )
    trigger_entry_ids: List[str] = Field(
        default_factory=list, description="Tier 2 trigger entry IDs in this cluster"
    )
    memory_element_ids: List[str] = Field(
        default_factory=list, description="Tier 1 atomic memory IDs in this cluster"
    )

    created_at: datetime = Field(
        default_factory=datetime.now, description="When this cluster was created"
    )
    cluster_size: int = Field(
        default=0, description="Total number of elements across all tiers"
    )

    def get_all_element_ids(self) -> List[str]:
        """Get all element IDs across all tiers."""
        return self.conversation_ids + self.trigger_entry_ids + self.memory_element_ids
...
class TieredMemoryGraph(BaseModel):
    ...
    memory_graph_path: Optional[str] = Field(
        default=None, description="Path to serialized MemoryGraph (tier 1 & 2)"
    )

    # New tiers
    conversations: Dict[str, ConversationBoundary] = Field(
        default_factory=dict, description="Tier 3: Conversation boundaries by ID"
    )
    semantic_clusters: Dict[str, SemanticCluster] = Field(
        default_factory=dict, description="Tier 4: Semantic clusters by ID"
    )
...
class RetrievalResult(BaseModel):
    ...
    drill_down_ids: List[str] = Field(
        default_factory=list, description="IDs of lower-tier elements for drill-down"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata (timestamps, tags, etc.)"
    )
...
class TieredRetrievalResults(BaseModel):
    ...
    total_results: int
    tiers_used: List[MemoryTier] = Field(description="Which tiers were queried")
    retrieval_strategy: str = Field(description="Strategy used for this retrieval")
