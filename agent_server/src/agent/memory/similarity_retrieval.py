...
def retrieve_memories_by_similarity(
    ...
    if total_entries == 0:
        ...
        logger.info(
            f"Memory similarity search completed in {elapsed:.3f}s - no old entries to search"
        )
        return []
    ...
    if not candidate_entries:
        elapsed = time.time() - start_time
        logger.info(
            f"Memory similarity search completed in {elapsed:.3f}s - no entries with embeddings found"
        )
        return []
    ...
    if similarities:
        score_info = ", ".join(
            [f"{score:.3f}" for _, score in similarities[:max_results]]
        )
