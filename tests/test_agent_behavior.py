...
from agent.agent_events import AgentTextEvent, ToolStartedEvent, ToolFinishedEvent, ToolResultType, AgentErrorEvent
...
class TestAgentBehavior:
    ...
    def test_iteration_limit_enforcement(self):
        ...
        def mock_responses(*args, **kwargs):
            ...
            if call_count >= agent.config.max_iterations:
                ...
                return iter([
                    ...
                    {"message": {"content": f'{{"message": "iteration {call_count}"}}'}}
                ])
        ...
        assert len(tool_calls) == agent.config.max_iterations - 1  # Final iteration has no tools
    ...
    def test_context_info_api(self):
        ...
        assert updated_context.conversation_messages > context_info.conversation_messages
    ...
    def test_tool_execution_failure_handling(self):
        ...
        assert tool_finished_events[0].result_type == ToolResultType.ERROR
        assert "cause failure" in tool_finished_events[0].result
