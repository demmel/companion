...
from agent.types import TextToolContent, ToolCallSuccess, ToolResult
...
class MockTool(BaseTool):
    ...
    def run(self, agent, input_data):
        return ToolCallSuccess(
            content=TextToolContent(text=f"Mock result: {input_data.message}")
        )
...
class StatefulMockTool(BaseTool):
    ...
    def run(
        self,
        agent,
        input_data: MockToolInput,
        tool_id: str,
        progress_callback: Callable[[Any], None],
    ) -> ToolResult:
        agent.set_state("last_tool_message", input_data.message)
        return ToolCallSuccess(
            content=TextToolContent(text=f"State updated with: {input_data.message}")
        )
...
def create_stateful_config():
    ...
    return AgentConfig(
        ...
        default_state={"test_mode": True, "initial_value": "test", "counter": 0},
    )
