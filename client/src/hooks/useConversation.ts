...
export function useConversation(
  ...
  const [currentResponse, setCurrentResponse] = useState<MessageBuilder | null>(
    null,
  );
    ...
    for (const event of events) {
      ...
      switch (event.type) {
        case "text":
          if (
            response &&
            (response.role !== "assistant" || response.tool_calls.length > 0)
          ) {
        ...
        case "tool_finished":
          ...
          if (!response || response.role !== "assistant") {
            console.warn(
              "Received tool_finished event but current response is not an assistant message. Ignoring.",
            );
            continue; // Ignore if current response is not an assistant message
          }

          const toolIndex = response.tool_calls.findIndex(
            (t) => t.tool_id === event.tool_id,
          );
          if (toolIndex === -1) {
            console.warn(
              `Tool finished event for unknown tool_id: ${event.tool_id}. Ignoring.`,
            );
        ...
        case "summarization_finished":
          ...
          if (!response || response.role !== "system") {
            console.warn(
              "Received summarization_finished event but current response is not a system message. Ignoring.",
            );
          ...
          if (!summarizationStartData) {
            console.warn(
              "Received summarization_finished event but no summarization start data found. Ignoring.",
            );
  ...
  const messages = useMemo(() => {
    ...
    return [...baseMessages, currentResponse as Message];
