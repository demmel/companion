...
import {
  UserBubble as UserBubbleComponent,
  AgentBubble as AgentBubbleComponent,
...
export function buildMessagesWithState(
  ...
  return messages.map((message, index) => {
    ...
    const currentCharacter = currentCharacterId
      ? currentState.characters[currentCharacterId]
      : null;
  ...
  switch (toolCall.tool_name) {
    ...
    case "set_mood": {
      if (
        ...
        newState.characters[newState.current_character_id]
      ) {
        ...
        newState.characters = {
          ...
          [newState.current_character_id]: {
            ...
            mood_intensity:
              toolCall.parameters.intensity || char.mood_intensity,
          },
        };
      }
      break;
...
export function groupMessagesIntoBubbles(
  messagesWithState: MessageWithState[],
): MessageBubble[] {
  ...
  for (const messageWithState of messagesWithState) {
    ...
    const systemTools = visibleToolCalls.filter((tc) =>
      SYSTEM_TOOLS.has(tc.tool_name),
    );
    const agentTools = visibleToolCalls.filter((tc) =>
      AGENT_TOOLS.has(tc.tool_name),
    );
  ...
  return (
    <div className={css({ mb: 4 })}>
      ...
      <AgentBubbleComponent>
        ...
        >
          {bubble.messages.map((messageWithState, index) => {
            const { message, visibleToolCalls, stateBeforeMessage } =
              messageWithState;
...
export function RoleplayPresenter({
  messages,
  isStreamActive,
  agentState,
  ...
  const messageBubbles = useMemo(() => {
    const messagesWithState = buildMessagesWithState(
      messagesToUse,
      agentState as RoleplayState,
    );
...
function SpecialToolPresentation({
  toolCall,
  stateBeforeMessage,
}: {
  toolCall: ToolCall;
...
function GeneratedImage({ toolCall }: { toolCall: ToolCall }) {
  ...
  if (content.type !== "image_generated") {
    return null;
  }
      ...
      {content.map((item, index) => {
        if (item.type === "text") {
          ...
          return (
            <SummarizationBubble
              key={index}
              content={item as SummarizationContent}
            />
          );
        }
        // Handle other content types if needed
...
function SystemToolPresentation({ toolCall }: { toolCall: ToolCall }) {
  ...
  switch (toolCall.tool_name) {
    ...
    case "remember_detail":
      return (
        <div>
          💾 Memory stored: {toolCall.parameters.detail || "Detail saved"}
        </div>
      );
    case "correct_detail":
      return (
        <div>
          ✏️ Memory updated:{" "}
          {toolCall.parameters.correction || "Detail corrected"}
        </div>
      );
    default:
      return null;
