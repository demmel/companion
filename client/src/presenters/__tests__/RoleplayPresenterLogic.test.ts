...
function createToolCall(
  ...
  return {
    ...
    tool_name: toolName,
    parameters,
    result: { type: "success", content: { type: "text", text: "Success" } },
  };
}
...
describe("RoleplayPresenter Logic Functions", () => {
  describe("buildMessagesWithState", () => {
    ...
    test.skip("visible tool calls count as content for header decision", () => {
      const messages = [
        ...
        createAgentMessage("", [
          // No text content, but has visible tool
  ...
  describe("groupMessagesIntoBubbles", () => {
    ...
    test.skip("mixed system and agent tools preserve order by splitting", () => {
      ...
      expect(
        bubbles[1].messages[0].visibleToolCalls.map((tc) => tc.tool_name),
  ...
  describe("Tool categorization and ordering", () => {
    test("tools are categorized correctly", () => {
      ...
      expect(
        messagesWithState[0].visibleToolCalls.map((tc) => tc.tool_name),
      ).toEqual([
        "scene_setting",
        "set_mood",
        "remember_detail",
